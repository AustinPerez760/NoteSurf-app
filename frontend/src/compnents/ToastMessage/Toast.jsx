		...
		<div
			...
			<div
				className={`min-w-52 bg-white border shadow-2xl rounded-md after:w-[5px] after:h-full ${
					type === 'delete' ? 'after:bg-red-400' : 'after:bg-blue-400'
				} after:absolute after:left-0 after:top-0 after:rounded-l-lg`}>
				<div className='flex items-center gap-3 py-2 px-4'>
					<div
						className={`w-10 h-10 flex items-center justify-center rounded-full ${
							type === 'delete' ? 'bg-teal-100' : 'bg-teal-100'
						}`}>
						{type === 'delete' ? (
							<MdDeleteOutline className='text-xl text-blue-500' />
						) : (
							<LuCheck className='text-xl text-blue-400' />
						)}
					</div>

					<p className='text-sm text text-slate-600'>{message}</p>
				</div>
			</div>
		</div>
	);
